...
const AllCachupReportByClerkTablesRecord = () => {
  ...
  const formatDate = date => {
    ...
    return `${year}${month}tbldata_cashup_det`
  }
  ...
  useEffect(() => {
    const validateDateRange = async () => {
      ...
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/database/allTblDataCashupDet`
  ...
  const exportToReport = () => {
    ...
    window.location.href = `/en/CachupReportByClerk_Report?${queryString}`
  }

  return (
    <Card className='p-4'>
      <CardHeader title={`${shopKey.toUpperCase()} Cachup Report By Clerk Data Table`} />
      <div className='p-6'>
...
export default AllCachupReportByClerkTablesRecord
