...
const AllHistoryInvoicesByStationTablesRecord = () => {
  ...
  const formatDate = date => {
    ...
    return `${year}${month}tbldata_current_tran`
  }
  ...
  useEffect(() => {
    const validateDateRange = async () => {
      ...
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/database/allTblDataCurrentTran`
  ...
  const exportToReport = () => {
    ...
    window.location.href = `/en/HistoryInvoicesByStation_Report?${queryString}`
  }

  return (
    <Card className='p-4'>
      <CardHeader title={`${shopKey.toUpperCase()} History Invoices By Station Data Table`} />
      <div className='p-6'>
...
export default AllHistoryInvoicesByStationTablesRecord
