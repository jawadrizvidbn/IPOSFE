...
const AllDebtorsCreditNotesTablesRecord = () => {
  ...
  const formatDate = date => {
    ...
    return `${year}${month}tbldebtor_tran`
  }
  ...
  useEffect(() => {
    const validateDateRange = async () => {
      ...
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/database/tblDataDebtorsTran`
  ...
  const exportToReport = () => {
    ...
    window.location.href = `/en/DebtorsCreditNotes_Report?${queryString}`
  }

  return (
    <Card className='p-4'>
      <CardHeader title={`${shopKey.toUpperCase()} Debtors Credit Notes Data Table`} />
      <div className='p-6'>
...
export default AllDebtorsCreditNotesTablesRecord
