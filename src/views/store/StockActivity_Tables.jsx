...
const AllStockActivityTablesRecord = () => {
  ...
  const formatDate = date => {
    ...
    return `${year}${month}tbldata_stockactivity`
  }
  ...
  useEffect(() => {
    const validateDateRange = async () => {
      ...
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/database/tblDataStockActivity`
  ...
  const exportToReport = () => {
    ...
    window.location.href = `/en/StockActivity_Report?${queryString}`
  }

  return (
    <Card className='p-4'>
      <CardHeader title={`${shopKey.toUpperCase()} Stock Activity Data Table`} />
      <div className='p-6'>
...
export default AllStockActivityTablesRecord
