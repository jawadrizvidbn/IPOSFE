...
const AllPayoutTablesRecord = () => {
  ...
  useEffect(() => {
    const validateDateRange = async () => {
      ...
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/database/tblPayout`
  ...
  const exportToReport = () => {
    ...
    window.location.href = `/en/payout_Report?${queryString}`
  }

  return (
    <Card className='p-4'>
      <CardHeader title={`${shopKey.toUpperCase()} Payout Data Table`} />
      <div className='p-6'>
...
export default AllPayoutTablesRecord
