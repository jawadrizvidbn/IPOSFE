...
const AllVoidTablesRecord = () => {
  ...
  const formatDate = date => {
    ...
    return `${year}${month}tbldata_cancel_tran`
  }
  ...
  useEffect(() => {
    const validateDateRange = async () => {
      ...
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/database/tblDataCancelTran`
  ...
  const exportToReport = () => {
    ...
    window.location.href = `/en/void_Report?${queryString}`
  }

  return (
    <Card className='p-4'>
      <CardHeader title={`${shopKey.toUpperCase()} Void Data Table`} />
      <div className='p-6'>
...
export default AllVoidTablesRecord
